...
export default async function MaterialDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const unit = translateValue(t.units, material.unit);
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2.5">
              <h2 className="text-lg font-semibold tracking-tight sm:text-[19px]">
                {material.name}
              </h2>
              <StockStatusBadge status={status} />
            </div>
            <p className="mt-1 text-[13px] text-muted-foreground">
              {translateValue(t.categories, material.category)} · {t.materials.detail.unitLabel}:{" "}
              {unit}
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard
          ...
          icon={Boxes}
          hint={t.materials.detail.minimumIs(qty(material.minStock))}
        ...
        <StatCard
          ...
          hint={
            holders.length > 0
              ? t.materials.detail.foremenCount(holders.length)
        ...
        <StatCard
          ...
          icon={TrendingDown}
          hint={t.materials.detail.returned(qty(totals.returned))}
          tone="neutral"
        />
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-0.5">
            ...
            {holders.map((holder) => (
              ...
              >
                <div className="min-w-0">
                  <div className="truncate text-[13px] font-medium">{holder.foremanName}</div>
      ...
      <div>
        <h3 className="mb-2.5 text-[13px] font-semibold">{t.materials.detail.history}</h3>
        <MovementsTable
          movements={movements}
          columns={[
            "type",
